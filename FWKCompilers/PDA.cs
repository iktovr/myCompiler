...
namespace Translator {
    ...
    class PDA : Automate //Push Down Automate МП = {}
    {
        ...
        public PDA(Grammar KCgrammar) {
            ...
            foreach (var v1 in KCgrammar.V) { // build DeltaQSigmaGamma
            ...
            foreach (var t1 in KCgrammar.T) {
                Stack e = new Stack();
                e.Push("e");
                delta=new DeltaQSigmaGamma(Q0,t1.symbol,t1.symbol,
                                                                     new List<Symbol>() { new Symbol(Q0) },
                                                                     new List<Symbol>() { new Symbol("e") });
                D.Add(delta);
            }
        }
        public virtual void addDeltaRule(
                                 string LHSQ,string LHSS,string LHSZ,
                                 List<Symbol> RHSQ,List<Symbol> RHSZ) {
            D.Add(new DeltaQSigmaGamma(LHSQ,LHSS,LHSZ,RHSQ,RHSZ));
        }
        public virtual void addDeltaRule(
                         string LHSQ,string LHSS,string LHSZ,
                         List<Symbol> RHSQ,List<Symbol> RHSZ,
                         List<Symbol> RightSix) {
            D.Add(new DeltaQSigmaGammaSix(LHSQ,LHSS,LHSZ,RHSQ,RHSZ,RightSix));
        }
    ...
    class translMp : PDA    //МП = {}
    {
        ...
        public translMp(List<Symbol> Q,List<Symbol> Sigma,List<Symbol> Gamma,string Q0,string Z0,List<Symbol> F)
        ...
        public translMp(Grammar KCgrammar): base(KCgrammar) {
            ...
            DeltaQSigmaGamma delta = null;
            foreach (var v1 in KCgrammar.V) {            // сопоставление правил с отображениями
                var q1 = new List<Symbol>();
                var z1 = new List<Symbol>();
            ...
            foreach (var t1 in KCgrammar.T) {
                var e = new Stack();
                e.Push("e");
                delta=new DeltaQSigmaGamma(Q0,t1.symbol,t1.symbol,new List<Symbol>() { new Symbol(Q0) },
                                                                                        new List<Symbol>() { new Symbol("e") });
                D.Add(delta);
            }
        }
