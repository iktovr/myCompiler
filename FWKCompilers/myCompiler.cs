...
namespace Translator {
    ...
    public abstract class Automate {
        ...
        public void AddRule(string state,string term,string nextState) {
            this.D.Add(new DeltaQSigma(state,term,
                       new List<Symbol> { new Symbol(nextState)}));
        }
        ...
        private List<Symbol> Dtran(List<Symbol> currState) {
            ...
            foreach (var a in Sigma) { 
                ...
                if (SetName(newState)!=null)
                    DeltaD.Add(new DeltaQSigma(SetName(currState), 
                                                 a.symbol,new List<Symbol> { new Symbol(SetName(newState)) }));
                debugDeltaRule("d",new DeltaQSigma(SetName(currState),a.symbol,
                    new List<Symbol> { new Symbol(SetName(newState)) }));
